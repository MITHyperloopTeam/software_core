...
int linux_cobs_serial_transport_sendmsg(zcm_trans_t *zt, zcm_msg_t msg){
    linux_cobs_serial_transport_t * t = (linux_cobs_serial_transport_t *) zt;
    ...
    while (msg.channel[channelNameLen] != 0) {
        ...
        channelNameLen+=1;
        if (channelNameLen > 255) // force to fit in one byte
            return ZCM_EINVALID;
    }
    if (msg.len > MTU)
        return ZCM_EINVALID;
...
{
    if (size > 0){
        linux_cobs_serial_transport_t * t = (linux_cobs_serial_transport_t *) extra;
        ...
        if (next_recv_buffer_head != t->recv_buffer_head && size <= BUFFER_SIZE){
...
{
    linux_cobs_serial_transport_t *trans = new linux_cobs_serial_transport_t();
    trans->recv_buffer_tail = 0;
    trans->recv_buffer_head = 0;
    /* construct trans here */

    trans->serial.setPacketHandler(&linux_cobs_serial_transport_on_packet, trans);
    trans->serial.begin(115200, port);
